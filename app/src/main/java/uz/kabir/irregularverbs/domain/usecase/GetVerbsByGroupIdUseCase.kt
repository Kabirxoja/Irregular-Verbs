...
class GetVerbsByGroupIdUseCase @Inject constructor(
    private val localVerbRepository: LocalVerbRepository,
    private val settingsRepository: SettingsRepository
) {
    suspend operator fun invoke(groupId: Int): List<IrregularVerbTranslated> {

        val languageCode = settingsRepository.getLanguageCode().first()
        val verbs = localVerbRepository.getVerbsByGroupId(groupId)
